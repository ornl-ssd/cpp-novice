...
can declare multiple variables of type `int`. For example, following 
with the previous example of class `Rectangle`, we could have declared the object `rectb` in addition to object `rect`:
...
In this case, the class (type of the objects) is `Rectangle`, of which there are two instances `rect` and `rectb`. Each one of them has its own 
